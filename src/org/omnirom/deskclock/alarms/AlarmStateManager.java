...
import org.omnirom.deskclock.AlarmPluginFactory;
import org.omnirom.deskclock.AlarmUtils;
import org.omnirom.deskclock.AsyncHandler;
...
public final class AlarmStateManager extends BroadcastReceiver {
    ...
    public static void setSilentState(Context context, AlarmInstance instance) {
        ...
        scheduleInstanceStateChange(context, instance.getHighNotificationTime(highNotificationOffset),
    ...
    /*public static void setLowNotificationState(Context context, AlarmInstance instance) {
        ...
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
        if (prefs.getBoolean(SettingsActivity.KEY_PRE_ALARM_NOTIFICATION_SHOW, true)) {
            // Setup instance notification
    ...
    public static void setPreFiredState(Context context, AlarmInstance instance) {
        ...
        // Start the alarm
        startAlarm(context, instance, true);
        scheduleInstanceStateChange(context, instance.getAlarmTime(),
                instance, AlarmInstance.FIRED_STATE);
    }
    ...
    public static void setSnoozeState(Context context, AlarmInstance instance, boolean showToast) {
        ...
        if (showToast) {
            String displayTime = String.format(context.getResources().getQuantityText
                    (org.omnirom.deskclock.R.plurals.alarm_alert_snooze_set, snoozeMinutes).toString(), snoozeMinutes);
    ...
    public static boolean canSnooze(Context context) {
    ...
    private static void startAlarm(Context context, AlarmInstance instance, boolean preAlarm) {
        final Intent intent = AlarmPluginFactory.getAlarmServiceIntent(context, instance, preAlarm);
        if (intent == null) {
            LogUtils.v("AlarmStateManager startAlarm: failed to get start intent " + instance);
            return;
        }
