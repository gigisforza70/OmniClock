...
import org.omnirom.deskclock.VerticalViewPager;
...
public class TimerFragment extends DeskClockFragment implements OnSharedPreferenceChangeListener {
                    ...
                    if (t.mView != null) {
                        ...
                        if (toggle) {
                            final ImageButton addMinuteButton = (ImageButton)
                                    t.mView.findViewById(org.omnirom.deskclock.R.id.reset_add);
    ...
    private void setTimerViewFabIcon(TimerObj timer) {
        ...
        switch (timer.mState) {
            case TimerObj.STATE_RUNNING:
                ...
                mFab.setContentDescription(r.getString(org.omnirom.deskclock.R.string.timer_stop));
            ...
            case TimerObj.STATE_RESTART:
                mFab.setVisibility(View.VISIBLE);
                mFab.setContentDescription(r.getString(org.omnirom.deskclock.R.string.timer_start));
            ...
            case TimerObj.STATE_TIMESUP: // time-up but didn't stopped, continue negative ticking
                mFab.setVisibility(View.VISIBLE);
                mFab.setContentDescription(r.getString(org.omnirom.deskclock.R.string.timer_stop));
                mFab.setImageResource(org.omnirom.deskclock.R.drawable.ic_cancel);
                break;
            default:
        }
    }
    ...
    public void setFabAppearance() {
        ...
        if (mLastView == mTimerView) {
            ...
            mFab.setImageResource(org.omnirom.deskclock.R.drawable.ic_fab_play);
            mFab.setContentDescription(getString(org.omnirom.deskclock.R.string.timer_start));
        }
    }
        ...
        if (count <= PAGINATION_DOTS_COUNT) {
            for (int i = 0; i < PAGINATION_DOTS_COUNT; i++) {
                if (count < 2 || i >= count) {
                    ...
                    paintIndicator(i, position == i ? org.omnirom.deskclock.R.drawable.ic_swipe_circle_light_black :
                            org.omnirom.deskclock.R.drawable.ic_swipe_circle_dark_black);
                }
            }
        } else {
            /**
            ...
            if (aboveCount < PAGINATION_DOTS_COUNT - 1) {
                ...
                for (int i = 0; i < aboveCount; i++) {
                    paintIndicator(i, org.omnirom.deskclock.R.drawable.ic_swipe_circle_dark_black);
                }
                paintIndicator(position, org.omnirom.deskclock.R.drawable.ic_swipe_circle_light_black);
                for (int i = position + 1; i < PAGINATION_DOTS_COUNT - 1 ; i++) {
                    paintIndicator(i, org.omnirom.deskclock.R.drawable.ic_swipe_circle_dark_black);
                }
                paintIndicator(PAGINATION_DOTS_COUNT - 1, org.omnirom.deskclock.R.drawable.ic_swipe_circle_bottom_black);
            } else {
                // There's not enough room for the above timers, top dot needs to fade
                paintIndicator(0, org.omnirom.deskclock.R.drawable.ic_swipe_circle_top_black);
                ...
                paintIndicator(PAGINATION_DOTS_COUNT - 2, belowCount == 0 ?
                        org.omnirom.deskclock.R.drawable.ic_swipe_circle_dark_black : org.omnirom.deskclock.R.drawable.ic_swipe_circle_light_black);
                final int lastDotRes;
                if (belowCount == 0) {
                    // The current timer is the last one
                    lastDotRes = org.omnirom.deskclock.R.drawable.ic_swipe_circle_light_black;
                } else if (belowCount == 1) {
                    // There's only one timer below the current
                    lastDotRes = org.omnirom.deskclock.R.drawable.ic_swipe_circle_dark_black;
                } else {
                    // There are more than one timer below, bottom dot needs to fade
                    lastDotRes = org.omnirom.deskclock.R.drawable.ic_swipe_circle_bottom_black;
                }
